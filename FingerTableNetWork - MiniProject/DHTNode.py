...
class FingerTable:
    ...
    def refresh(self):
        ...
        for i in range(self.m_bits):
            ...
            address = self.finger_table[i][1]
            refreshed.append((i+1, node, address))
        return refreshed

    def getIdxFromId(self, id):
        for i in range(self.m_bits):
            node = (self.node_id + 2 ** i) % (2 ** self.m_bits)
            if contains(self.node_id, node, id):
                return i+1
...
class DHTNode(threading.Thread):
    ...
    def node_join(self, args):
        ...
        if self.identification == self.successor_id:  # I'm the only node in the DHT
            ...
            for i in range(self.finger_table.m_bits):
                id = (self.identification + 2 ** i) % (2 ** self.finger_table.m_bits)
    ...
    def get_successor(self, args):
        ...
        if contains(self.identification, self.successor_id, id):
            ...
            self.send(self.finger_table.find(id), {"method": "SUCCESSOR", "args": args})
    ...
    def stabilize(self, from_id, addr):
        ...
        for i in range(len(fingerTable_refreshed)):
            self.get_successor({"id": fingerTable_refreshed[i][1], "from": self.addr})
