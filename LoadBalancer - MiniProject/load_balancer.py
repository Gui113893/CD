...
class RoundRobin:
    def __init__(self, servers):
        self.servers = servers
        self.index=0
        self.lenServer=len(servers)
    def select_server(self):
        index=self.index
        self.index+=1
        if(self.index>=self.lenServer):
            self.index=0
...
class LeastConnections:
    def __init__(self, servers):
        self.servers = servers
        self.connections={i:0 for i in servers}
    def select_server(self):
        minVaule=list(self.connections.values())[0]
        minKey=list(self.connections.keys())[0]
        for key,vaule in self.connections.items():
            if minVaule>vaule:
                minVaule=vaule
                minKey=key
        self.connections[minKey]+=1
        return minKey
    def update(self, *arg):
        for i in arg:
...
class LeastResponseTime:
    def __init__(self, servers):
        self.servers = servers
        self.connections={i:[1,0,0] for i in servers}
    def select_server(self):
        timeStart=time.time()
        betterMedia = list(self.connections.values())[0][2]
        betterKey=list(self.connections.keys())[0]
        for key,vaule in self.connections.items():
            media=(vaule[1]+vaule[2]-timeStart)/vaule[0]
            if betterMedia>media:
                betterMedia=media
                betterKey=key
        self.connections[betterKey][2]=timeStart
        return betterKey
    def update(self, *arg):
        for i in arg:
            values = self.connections[i]
            timeFinal, timeStart =  values[1], values[2]
            self.connections[i][0]+=1
            self.connections[i][1]=(timeFinal+time.time()-timeStart)
